                ...
                <div className='mx-[20%]'>
                    {
                        results.map((v, i) => {
                            ...
                            return (
                                <div key={`Result-${i}`} className='my-8'>
                                    <hr />
                                            ...
                                            <Link href={`/video?v=${v.id}`} className='group/videoLongCard'>
                                                <div className='h-full w-full my-4 flex flex-row gap-2'>
                                                    <div className='relative aspect-video h-full'>
                                                        <Image className='rounded-lg' src={getImageSrcFromPath((v.thumbnail as Media).src)} alt={v.title} fill />
                                                    </div>
                                                    <div className='flex flex-col gap-2'>
                                                        <h3 className='group-hover/videoLongCard:text-primary !m-0 overflow-ellipsis line-clamp-3 text-wrap hyphens-auto break-all'>{v.title}</h3>
                                                    ...
                                                    <div className='flex flex-col h-fit gap-2'>
                                                        <div className='flex flex-row gap-2 w-fit'><FaThumbsUp className='w-[1em] h-full' />{v.likes.length}</div>
                                                        <div className='w-[2px] h-full bg-white rounded-lg' />
                                                        <div className='flex flex-row gap-2 w-fit'><FaThumbsDown className='w-[1em] h-full' />{v?.dislikes.length}</div>
                                                    </div>
                                                </div>
                                            </Link>
                                            :
